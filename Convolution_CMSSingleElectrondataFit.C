...
void Convolution_CMSSingleElectrondataFit(){
  RooRealVar x("x","e^{+} e^{-} Inv. Mass (in GeV)",75,120,"GeV");
  TFile *inputfile = new TFile("MyAn_data_SingleElectron.root","READ");
  inputfile->cd();
  
  TH1F *heg  = (TH1F*)inputfile->Get("h1");
  RooDataHist dh("dh","dh",x,Import(*heg)) ;
  RooPlot* frame = x.frame(Title("M_{e^{+} e^{-}}")) ;
  ...
  RooRealVar nsig("nsig","signal events",50000,0,8000000);
  RooRealVar nbkg("nbkg","signal background events",1000,0,2000000);

  RooAddPdf  model("model","bwcball+shape",RooArgList(bwcball,shape),RooArgList(nsig,nbkg)) ; 

  //model.fitTo(dh,Range(60,120));                                                                                
  model.fitTo(dh,Extended(),Range(75,120));                                                                                
  // model.plotOn(frame); 
  ...
  std::cout <<"Chi2/dof first = " << chi2.getVal() << std::endl ;
  
  // Double_t chi2 = model.GetChisquare(); // chi square of fit
  // Double_t p11 = model.GetParameter(0); // p value of fit
  cout<<"Second Fit chi square/dof = "<<frame->chiSquare(7)<<endl;
  // c->SaveAs("hello.png")  ;
  //  inputfile->Close();
